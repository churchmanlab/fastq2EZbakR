...
# Two strategies (set via `snp_strategy` in the config): # Added in _MTC
#   1) genome_likelihoods = bcftools call's genotype-likelihood variant model (original behavior)
#   2) counts_in_control  = mark any site where at least `snp_threshold` reads in the control show a mismatch
if config.get("snp_strategy", "genome_likelihoods") == "genome_likelihoods":
